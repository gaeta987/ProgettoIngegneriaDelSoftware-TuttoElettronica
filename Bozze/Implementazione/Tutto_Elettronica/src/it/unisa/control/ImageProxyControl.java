...
import it.unisa.proxy.ProxyImage;
...
public class ImageProxyControl extends HttpServlet {
  private static final long serialVersionUID = 1L;

  private static final String SAVE_DIR = 
      "C:\\Users\\pc\\eclipse-workspace\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\Tutto_Elettronica\\images\\copertine\\";

  public ImageProxyControl() {
    super();
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) 
    ...
    if (name != "") {
      if (iconPath.toFile().exists()) {
        ...
        iconPath = Paths.get(realPath(request)).resolve(SAVE_DIR).resolve("default.png");
        f = iconPath.toFile();
        prox = new ProxyImage(iconPath);
      }
    } else {
